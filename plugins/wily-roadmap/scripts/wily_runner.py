...
def render_custom_workflow_request(
    ...
    return "\n".join(
        [
            ...
            "- After verification evidence exists, the active agent should run `$wily-complete <phase-id>` or `$wily-block <phase-id> \"<reason>\"`.",
            "",
            "## Phase Context",
            "",
            phase_context.strip(),
...
def command_run(root: Path, args: list[str]) -> int:
    ...
    artifacts = write_custom_workflow_request(root, phase, workflow, engine, autonomy_mode, session)
    ...
    print(f"Prepared phase {phase_id} for Custom Workflow Skillset")

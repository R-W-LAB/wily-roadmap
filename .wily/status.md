...
- Child Phase 15-7 opened draft workflow PRs for all four initial repositories and configured `WILY_BOARD_URL` secrets, then blocked until Azure SSH access and remaining live credentials exist.
...
- Merge the workflow PRs after `WILY_BOARD_SECRET` is available as a repo or org secret:

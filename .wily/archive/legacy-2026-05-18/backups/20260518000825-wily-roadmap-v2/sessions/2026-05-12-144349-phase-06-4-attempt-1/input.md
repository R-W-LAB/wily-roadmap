...
Use the planner because init behavior is foundational and user-facing.

## Prompt

# Execution Prompt

Define and implement the `$wily-init` contract for mature repositories.

Scope:
- Decide expected behavior for existing codebases with no `.wily`.
- Decide expected behavior for partial or existing `.wily`.
- Preserve user-authored roadmap/status/decision files.
...
Also manually inspect `$wily-init` output in a temporary mature-looking repository fixture.

## Handoff

# Handoff

Start by capturing current `$wily-init` behavior for an empty repo, a mature repo with files, and a partial `.wily` directory.

This phase can run in parallel with phases 06-1, 06-2, 06-3, and 06-5.
...
Generate one before changing init behavior.

...
Red result: failed before implementation because `live_items.cleared_at` was not updated and `current_item_id` was ignored.
...
Result: no runtime live fields; only the human task text mentions `session_id`.

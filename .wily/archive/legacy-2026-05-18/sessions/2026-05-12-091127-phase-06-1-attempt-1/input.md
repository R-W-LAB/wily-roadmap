...
Use the planner because this phase changes user-facing skill response contracts across multiple skills.

## Prompt

# Execution Prompt

Reduce unnecessary output from Wily skills.

Scope:
- Review every `skills/wily-*/SKILL.md` response section.
- Keep safety-critical approval-first guidance.
- Move repeated policy to references when useful.
- Add focused tests for the expected concise response contracts.
...
Manually inspect representative skill files for concise, non-duplicative response guidance.

## Handoff

# Handoff

Start by listing the current response-style bullets in every `skills/wily-*/SKILL.md` file and grouping duplicate guidance.

This phase can run in parallel with phases 06-2 through 06-5.
...
Generate one before editing skill response contracts.
